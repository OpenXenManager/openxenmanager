...
class oxcSERVERaddserver(gobject.GObject):
    ...
    def connect_server(self):
        if self.ssl:
            self.connection = xmlrpclib.Server("https://%s" % self.host)
            self.connection_events = xmlrpclib.Server("https://%s" % self.host)
        else:
            self.connection = xmlrpclib.Server("http://%s" % self.host)
            self.connection_events = xmlrpclib.Server("http://%s" % self.host)
        try:
    ...
    def sync(self):
        # Get all vm records
        self.emit("sync-progress", "Retrieving records")
